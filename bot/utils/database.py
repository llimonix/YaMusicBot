...
class DataBase:
    ...
    async def insert_server(server_id, channel_id):
        async with aiosqlite.connect("servers.db") as conn:
            await conn.execute("INSERT INTO servers (id, info) VALUES (?, ?)", (server_id, channel_id))
            await conn.commit()

    @staticmethod
    async def insert_role(server_id, role_id):
        async with aiosqlite.connect("servers.db") as conn:
            await conn.execute("INSERT INTO servers (id, role) VALUES (?, ?)", (server_id, role_id))
            await conn.commit()

    @staticmethod
    async def update_role(server_id, role_id):
        async with aiosqlite.connect("servers.db") as conn:
            await conn.execute("UPDATE servers SET role = ? WHERE id = ?", (role_id, server_id))
            await conn.commit()

    @staticmethod
    async def insert_info(server_id, info_id):
        async with aiosqlite.connect("servers.db") as conn:
            await conn.execute("INSERT INTO servers (id, info) VALUES (?, ?)", (server_id, info_id))
            await conn.commit()

    @staticmethod
    async def update_info(server_id, info_id):
        async with aiosqlite.connect("servers.db") as conn:
            await conn.execute("UPDATE servers SET info = ? WHERE id = ?", (info_id, server_id))
            await conn.commit()

    @staticmethod
    async def select_role(server_id):
        async with aiosqlite.connect("servers.db") as conn:
            async with conn.execute("SELECT role FROM servers WHERE id = ?", (server_id,)) as cursor:
