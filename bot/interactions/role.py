...
class SetRoleCommand(commands.Cog):
    ...
    @commands.slash_command(name="setrole", name_localizations={"ru": "set-role"},
                            description="The role to which the bot will respond",
                            description_localizations={
                                "ru": "Роль на которую будет отвечать бот"
                            }
                            )
    @option(name="role", name_localizations={"ru": "роль"},
            description="The role to which the bot will respond",
            description_localizations={
                "ru": "Роль на которую будет отвечать бот"
            }
    ...
    async def setrole(self, ctx: ApplicationContext, role: Role):
        ...
        await ctx.respond(
            content=f"Установлена роль для управления ботом: {role.mention}",
            ephemeral=True
        )
