...
from discord import option, OptionChoice, ApplicationContext, Embed, Colour, ClientException, guild_only
...
class PlayCommand(commands.Cog):
    ...
    @commands.slash_command(
        name="play", description="Add a song to the queue", description_localizations={"ru": "Добавить песню в очередь"}
    )
    @option(
        name="query",
        name_localizations={"ru": "ссылка-или-название"},
        description="Track Name/Playlist/Album/Podcast/Books or a link (Yandex.Music)!",
        description_localizations={"ru": "Название Трека/Плейлиста/Альбома/Подкаста/Книги или ссылка(Яндекс.Музыка)!"},
    )
    @option(
        name="modes",
        name_localizations={"ru": "режим"},
    ...
    async def play(self, ctx: ApplicationContext, *, query: str, modes: int = 2) -> None:
        ...
        await ctx.respond(embed=Embed(title="Ожидайте...", color=Colour.yellow()), ephemeral=True, delete_after=15)
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            type_search = {"artist": "Плейлист", "playlist": "Плейлист", "album": "Альбом"}.get(tracks.type, "")
            await PlayCommand.add_to_queue_message(
                ctx=ctx, description=f"{type_search} **`{tracks.name}`** ({tracks_len})"
            )
        else:
            track = tracks[0]
            track.extras = {
            ...
            await PlayCommand.add_to_queue_message(ctx=ctx, description=f"Трек **`{track}`**")
    ...
    @commands.slash_command(
        name="stop",
        description="Stop playing music or fix bot freezes",
        description_localizations={"ru": "Остановить проигрывание музыки или пофиксить зависание бота"},
    )
    @guild_only()
    async def stop(self, ctx: ApplicationContext) -> None:
        ...
        if not player:
            await ctx.respond(
                embed=Embed(title="Ошибка", description="Бот нигде не запущен!", color=Colour.red()),
                delete_after=10,
                ephemeral=True,
            )
            return

        if (ctx.user.voice is None or player.channel != ctx.user.voice.channel) and ctx.guild.get_member(
            self.bot.user.id
        ).voice is not None:
            await ctx.respond(
                embed=Embed(
    ...
    async def add_to_queue_message(ctx: ApplicationContext, description: str) -> None:
        await ctx.edit(embed=Embed(colour=Colour.green(), description=f"{description} добавлен в очередь."))
        await ctx.channel.send(
            content=f"{ctx.user.mention} добавил в очередь {description}!", delete_after=10, silent=True
        )
