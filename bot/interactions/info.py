from discord import option, TextChannel, ApplicationContext, guild_only
...
class SetInfoCommand(commands.Cog):
    ...
    @commands.slash_command(
        name="setinfo",
        name_localizations={"ru": "set-info"},
        description="The channel to which the bot will send information about new updates",
        description_localizations={"ru": "Канал, в который бот будет отправлять информацию о новых обновлениях"},
    )
    @option(
        name="channel",
        name_localizations={"ru": "канал"},
        description="Channel for updates",
        description_localizations={"ru": "Канал для обновлений"},
    )
    @guild_only()
    async def setinfo(self, ctx: ApplicationContext, channel: TextChannel):
        ...
        await ctx.respond(content=f"Установлен канал для уведомлений о обновлениях: {channel.mention}", ephemeral=True)
